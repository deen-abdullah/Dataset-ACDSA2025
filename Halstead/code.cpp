...
void trivial(const string& t, const int i, vi& ans)
{
	int n = sz(t);
	while (i - ans[i] > 0 && i + ans[i] < n - 1 && t[i - ans[i] - 1] == t[i + ans[i] + 1])
		++ans[i];
}
vi real_manaker(const string& t)
{
	vi ans(sz(t));
	int n = sz(t);
	int r = 0;
	...
	{
		if (i <= r)
			ans[i] = min(min(i, n - i - 1), min(ans[2 * c - i], r - i));
		trivial(t, i, ans);
		if (i + ans[i] > r)
		{
			r = i + ans[i];
...
vi manaker(const string& s)
{
	string t;
	for (int i = 0; i < sz(s); ++i)
		t += string(1, '$') + s[i];
	t += '$';
	return real_manaker(t);
}
string find_ans(const string& s)
{
	int n = sz(s);
	int i = 0, j = n - 1;
	while ((i < j) && (s[i] == s[j]))
